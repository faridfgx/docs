...
function checkSyntax(code) {
    ...
    for (let i = 0; i < lines.length; i++) {
		...
		// 2b. Inline Var declaration: "Var x: entier;" or "Var a, b: reel; nom: chaine;"
		if (line.startsWith('var ')) {
			...
			// Parse inline declarations separated by ";"
			const inlineDecl = line.substring(4).trim();
			const declSegments = inlineDecl.split(';').map(s => s.trim()).filter(Boolean);

			for (const seg of declSegments) {
				const segMatch = seg.match(
					/^([a-z_][a-z0-9_]*(?:\s*,\s*[a-z_][a-z0-9_]*)*)\s*:\s*([a-z]+)\s*$/i
				);

				if (!segMatch) {
					return createError(
						'INVALID_VAR_DECLARATION', lineNum, line,
						'Déclaration de variable invalide',
						'Format attendu: Var nom: type; ou Var a, b: type;',
						'Var x: entier;\nVar a, b: reel; nom: chaine;'
					);
				}

				const varNames = segMatch[1].split(',').map(v => v.trim());
				const varType = segMatch[2].toLowerCase();

				if (!validTypes.includes(varType)) {
					return createError(
						'INVALID_TYPE', lineNum, line,
				...
				for (const varName of varNames) {
					...
					if (declaredConsts.has(varName)) {
						return createError(
							'VAR_CONST_CONFLICT', lineNum, line,
			...
			for (const seg of declSegments) {
				...
				if (!segMatch) {
					return createError(
						...
						line,
						'Déclaration de variable invalide',
						'Format attendu: nomVariable: type; ou x: entier; y: reel;',
						'x, y: entier;\nnom: chaine;\na: entier; b: reel;'
					);
				}
	...
	if (!hasVar) {
		return createError(
			...
			1,
			'',
			'Section "Var" manquante',
			'La section "Var" est obligatoire. Vous pouvez la déclarer seule sur une ligne, ou avec des variables sur la même ligne.',
			'Algorithme MonAlgorithme;\nVar\n    n: entier;\nDebut\n    // code\nFin'
		);
	}
    if (blockStack.length > 0) {
