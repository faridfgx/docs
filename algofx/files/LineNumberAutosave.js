...
let pendingAutoSaveDecision = false; // NEW: Block auto-save until user decides
...
function checkAutoSaveOnLoad() {
    ...
    if (savedCode && savedCode.trim() !== '') {
        pendingAutoSaveDecision = true; // Block auto-save until decision
...
function restoreAutoSave() {
    ...
    removeRestoreBanner();
    pendingAutoSaveDecision = false; // Re-enable auto-save
...
function setupAutoSave() {
    ...
    editor.addEventListener('input', () => {
        // Only auto-save if:
        // 1. No pending restore decision
        // 2. Not an opened file
        if (!pendingAutoSaveDecision && !isOpenedFile) {
...
function setupBeforeUnload() {
    window.addEventListener('beforeunload', (e) => {
        ...
        if (currentCode.trim() !== '' && currentCode !== lastManualSave) {
            // Save to localStorage before leaving (only if no pending decision)
...
if (typeof originalSaveFile === 'function') {
    window.saveFile = function() {
        ...
        clearAutoSaveStorage();
        isOpenedFile = false;
        pendingAutoSaveDecision = false; // Clear any pending decision
