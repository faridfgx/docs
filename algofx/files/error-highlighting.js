...
class AlgoErrorHighlighter {
    constructor() {
        ...
        this.standaloneKeywords = new Set([
            'const', 'debut', 'fin', 'sinon', 'finsi', 'finpour', 'fintantque'
        ]);
        ...
        lines.forEach((rawLine, i) => {
            ...
            if (line.startsWith('var')) {
                ...
                if (afterVar.length === 0) {
                    // 'Var' alone on its line — correct, nothing more to check
                    return;
                }

                // Inline Var: "Var x: entier;" or "Var a, b: reel; nom: chaine;"
                // Split on ";" to handle multiple declarations on one line
                const validTypes = ['entier', 'reel', 'chaine', 'caractere', 'booleen', 'tableau'];
                const segments = afterVar.split(';').map(s => s.trim()).filter(Boolean);

                for (const seg of segments) {
                    const segMatch = seg.match(/^([a-z_][a-z0-9_,\s]*)\s*:\s*([a-z]+)\s*$/i);
                    if (!segMatch) {
                        this.addError(lineNum, 'Déclaration variable invalide (format: Var nom: type;)', 'error');
                        return;
                    }
...
function showToast(message, type) {
  ...
  const accent = {success:'#10b981', error:'#ef4444', info:'#3b82f6'}[type] || '#3b82f6';
  ...
  Object.assign(toast.style, {
    ...
    zIndex:'2147483647', maxWidth:'min(420px,90vw)',
